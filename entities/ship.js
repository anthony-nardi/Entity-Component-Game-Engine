moduleLoader.imports('ship', ['unit','mainView', 'vector', 'collision'], function (unit, mainView, vector, collision) {
  ...
  var ship = Object.create(unit).inputs().extend({
    ...
    'move': function () {
      this.normalize();
      this.scale(this.currentSpeed);
      this.x += this.vx;
      this.y += this.vy;
      mainView.scroll.x += (this.vx * mainView.zoom);
      mainView.scroll.y += (this.vy * mainView.zoom);
    },
    ...
    if (this.state.render) {
      ...
      this.ctx.rotate(this.toRadians());

      this.ctx.fillRect(
        0 - ((this.getWidth() * mainView.zoom) / 2),
        0 - ((this.getHeight() * mainView.zoom) / 2),
        this.width * mainView.zoom, 
        this.height * mainView.zoom
      );
      this.ctx.restore();
